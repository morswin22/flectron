...
namespace flectron 
{
  ...
  bool World::removeBody(size_t index)
  {
  ...
  Ref<Body>& World::getBody(size_t index)
  {
    if (index >= bodyList.size())
  ...
  {
    FLECTRON_PROFILE_EVENT("World::update");
    FLECTRON_PROFILE_TAG("bodyList.size", bodyList.size());
    if (bodyList.empty())
      return;
  ...
  void World::resolveCollision(Ref<Body>& bodyA, Ref<Body>& bodyB, const Vector& normal, float)
  {
