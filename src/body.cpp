...
namespace flectron 
{
  ...
  unsigned int* trianglesFromVertices(Vector* vertices, unsigned int numVertices)
  {
    if (numVertices < 3)
    ...
    while (indexList.size() > 3)
    {
      for (unsigned int i = 0; i < indexList.size(); i++)
      {
        ...
        unsigned int b = indexList[getIndex(i - 1, (int)indexList.size())];
        unsigned int c = indexList[getIndex(i + 1, (int)indexList.size())];
