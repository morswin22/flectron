...
namespace flectron { namespace WFC{
      ...
      weights(), config(config), propagator(nullptr), tiles(nullptr), tileSize(32), tileMap()
  {
    ...
    for (size_t t1 = 0; t1 < numPatterns; ++t1) {
      for (size_t t2 = 0; t2 < numPatterns; ++t2) {
      ...
      for (size_t y2 = 0; y2 < height; ++y2)
        for (size_t d = 0; d < 4; ++d) 
        {
          ...
          for (size_t t2 = 0; t2 < numPatterns; ++t2) 
          {
            ...
            {
              bool b = false;
              for (size_t t1 = 0; t1 < numPatterns && !b; ++t1) 
              {
  ...
  Result TileModel::observe(Output* output, RandomDouble& randomDouble) const
  {
    ...
    for (size_t t = 0; t < numPatterns; ++t) {
  ...
  std::vector<std::vector<Tile*>> TileModel::getTiles(Ref<Output>& output)
  {
    ...
    for (size_t i = 0; i < width; ++i)
    {
      ...
      {
        int selected = -1;
        for (size_t k = 0; k < numPatterns; ++k)
        {
